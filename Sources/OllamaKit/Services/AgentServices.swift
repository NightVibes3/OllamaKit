...
final class BrowserSessionManager: NSObject, ObservableObject {
    ...
    func openTab(urlString: String?) async -> BrowserSessionRecord {
        ...
        AgentLogStore.shared.record(.browser, title: "Browser Tab Opened", message: target)
    ...
    func navigate(sessionID: UUID?, urlString: String) async throws -> BrowserSessionRecord {
        ...
        await waitForNavigation(for: record.id)
        await refreshSessionState(sessionID: record.id)
        AgentLogStore.shared.record(.browser, title: "Browser Navigate", message: url.absoluteString)
    ...
    func savePageToWorkspace(sessionID: UUID?, workspaceID: String?, relativePath: String?) async throws -> AgentJSONValue {
        ...
        AgentLogStore.shared.record(.browser, title: "Browser Page Saved", message: path)
        return .object([
    ...
    func download(
        ...
        AgentLogStore.shared.record(.browser, title: "Browser Download Saved", message: filename, metadata: ["url": url.absoluteString])
        return .object([
    ...
    func capture(sessionID: UUID?, workspaceID: String?, relativePath: String?) async throws -> AgentJSONValue {
        ...
        AgentLogStore.shared.record(.browser, title: "Browser Capture Saved", message: path)
        return .object([
            "workspace_path": .string(path),
            "bytes": .int(data.count)
        ])
...
extension BrowserSessionManager: WKNavigationDelegate, WKUIDelegate {
    ...
    func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        ...
        persist()
        navigationContinuations.removeValue(forKey: sessionID)?.resume()
        AgentLogStore.shared.record(.browser, level: .error, title: "Browser Navigation Failed", message: error.localizedDescription)
    }
...
final class AgentApprovalCenter: ObservableObject {
    ...
    func approve(id: UUID) async -> AgentExecutionResult {
        ...
        AgentLogStore.shared.record(
            ...
            metadata: ["tool": pending.request.toolID]
        )
...
final class AgentToolRuntime {
    ...
    private func capabilityFailureReason(
        ...
        let labels = missingCapabilities
            ...
            .joined(separator: ", ")
        return "\(model.displayName) is currently blocked from \(labels) in conservative agent mode."
    }
    ...
    func toolDescriptors(for model: ModelSnapshot? = nil) -> [AgentToolDescriptor] {
        ...
        return [
            ...
            AgentToolDescriptor(id: "browser.open_tab", title: "Open Browser Tab", category: .browser, detail: "Open a new embedded browser tab.", networkScope: .readOnly, writeScope: .none, needsApproval: false, available: browserAvailable, availabilityReason: browserAvailable ? nil : "WKWebView is unavailable on this build.", timeoutSeconds: 20, quotaDescription: "Creates a tab"),
            AgentToolDescriptor(id: "browser.navigate", title: "Navigate Browser", category: .browser, detail: "Navigate the active browser tab to a URL.", networkScope: .readOnly, writeScope: .none, needsApproval: false, available: browserAvailable, availabilityReason: browserAvailable ? nil : "WKWebView is unavailable on this build.", timeoutSeconds: 30, quotaDescription: "Single navigation"),
            ...
            AgentToolDescriptor(id: "browser.download", title: "Download Browser Asset", category: .browser, detail: "Download a URL from the active browser session into a workspace.", networkScope: .sideEffecting, writeScope: .workspace, needsApproval: true, available: browserAvailable, availabilityReason: browserAvailable ? nil : "WKWebView is unavailable on this build.", timeoutSeconds: 60, quotaDescription: "Single download"),
            AgentToolDescriptor(id: "browser.capture", title: "Capture Browser", category: .browser, detail: "Take a snapshot of the active browser page.", networkScope: .readOnly, writeScope: .workspace, needsApproval: false, available: browserAvailable, availabilityReason: browserAvailable ? nil : "WKWebView is unavailable on this build.", timeoutSeconds: 20, quotaDescription: "PNG capture"),
            ...
            AgentToolDescriptor(id: "javascript.run", title: "Run JavaScript", category: .javascript, detail: "Execute a JavaScript snippet with JavaScriptCore.", networkScope: .none, writeScope: .none, needsApproval: false, available: jsAvailable, availabilityReason: jsAvailable ? nil : "JavaScriptCore is unavailable on this build.", timeoutSeconds: 10, quotaDescription: "Inline scripts"),
    ...
    func execute(toolID: String, arguments: AgentJSONValue) async -> AgentExecutionResult {
        ...
        guard let descriptor = toolDescriptors(for: resolvedAgentModel(arguments: args)).first(where: { $0.id == toolID }) else {
