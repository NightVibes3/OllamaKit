    ...
    private func handleGenerate(request: String, on connection: NWConnection) {
        ...
        Task {
            ...
            guard !model.localPath.isEmpty else {
                sendErrorResponse(status: 404, message: "Invalid model path", on: connection)
                return
            }
            
            guard model.fileExists else {
                sendErrorResponse(status: 404, message: "Model file not found. Please re-download the model.", on: connection)
                return
            }
            ...
            if !ModelRunner.shared.isLoaded {
                do {
                    ...
                    sendErrorResponse(status: 500, message: "Failed to load model: \(error.localizedDescription)", on: connection)
                    return
                }
            }
...
// MARK: - ModelStorage
/// Actor for thread-safe model storage operations
actor ModelStorage {
    static let shared = ModelStorage()
    
    private var cachedModels: [DownloadedModel] = []
    private var lastFetch: Date = .distantPast
    
    private init() {}
    
    /// Get all downloaded models
    /// Uses caching to avoid frequent SwiftData queries
    func getAllModels() async -> [DownloadedModel] {
        // In a real implementation, this would query SwiftData
        // For now, return cached models
        return cachedModels.filter { $0.isDownloaded }
    }
    ...
    func getModel(name: String) async -> DownloadedModel? {
        ...
        return models.first { model in
            model.name == name || model.modelId == name || model.displayName == name
        }
    }
    
    /// Delete a model by name
    func deleteModel(name: String) async {
        if let model = await getModel(name: name) {
            // Delete the file
            if !model.localPath.isEmpty {
                try? FileManager.default.removeItem(atPath: model.localPath)
            }
            // In a real implementation, also delete from SwiftData
            cachedModels.removeAll { $0.id == model.id }
        }
    }
    
    /// Update the cached models (called when models are downloaded or deleted)
    func updateCachedModels(_ models: [DownloadedModel]) {
        cachedModels = models
        lastFetch = Date()
    }
}
