...
struct AgentView: View {
    ...
    @StateObject private var browserSessions = BrowserSessionManager.shared
    ...
    private var activeBrowserSession: BrowserSessionRecord? {
        browserSessions.activeSession()
    }
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    SurfaceSectionCard(title: "Browser") {
                        browserSection
                    }
        ...
        .task {
            ...
            browserSessions.bootstrapIfNeeded()
            BundlePatchStore.shared.bootstrapIfNeeded()
            await refreshContext()
            displayedLogs = agentLogs.entries
            browserURL = activeBrowserSession?.currentURL ?? settings.agentBrowserHomeURL
        }
    ...
    private var browserSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    ForEach(browserSessions.sessions) { session in
                        Button {
                            browserSessions.setActiveSession(id: session.id)
                            browserURL = session.currentURL
                        } label: {
                            Text(session.title.nonEmpty ?? session.currentURL)
                                .lineLimit(1)
                                .font(.system(size: 12, weight: session.id == activeBrowserSession?.id ? .semibold : .regular))
            ...
            HStack(spacing: 8) {
                TextField("https://example.com", text: $browserURL)
                ...
                Button("Go") {
                    Task {
                        do {
                            if let session = activeBrowserSession {
                                _ = try await browserSessions.navigate(sessionID: session.id, urlString: browserURL)
                                settings.agentBrowserLastURL = browserURL
                                await refreshContext()
                            }
                        } catch {
                            errorMessage = error.localizedDescription
                ...
                Button("New") {
                    Task {
                        _ = await browserSessions.openTab(urlString: browserURL.nonEmpty ?? settings.agentBrowserHomeURL)
                        browserURL = activeBrowserSession?.currentURL ?? browserURL
                    }
                }
            }
            .font(.system(size: 13, weight: .medium))
            ...
            HStack(spacing: 12) {
                Button("Back") {
                    Task {
                        do {
                            _ = try await browserSessions.back(sessionID: activeBrowserSession?.id)
                            browserURL = activeBrowserSession?.currentURL ?? browserURL
                        } catch {
                            errorMessage = error.localizedDescription
                        }
                ...
                Button("Forward") {
                    Task {
                        do {
                            _ = try await browserSessions.forward(sessionID: activeBrowserSession?.id)
                            browserURL = activeBrowserSession?.currentURL ?? browserURL
                        } catch {
                            errorMessage = error.localizedDescription
                        }
                ...
                Button("Reload") {
                    Task {
                        do {
                            _ = try await browserSessions.reload(sessionID: activeBrowserSession?.id)
                            browserURL = activeBrowserSession?.currentURL ?? browserURL
                        } catch {
                            errorMessage = error.localizedDescription
                        }
                ...
                Button("Save Page") {
                    Task {
                        do {
                            _ = try await browserSessions.savePageToWorkspace(
                                sessionID: activeBrowserSession?.id,
                                workspaceID: AgentWorkspaceManager.downloadedSiteWorkspaceID,
                                relativePath: nil
                            )
                        } catch {
                            errorMessage = error.localizedDescription
                ...
                Button("Capture") {
                    Task {
                        do {
                            _ = try await browserSessions.capture(
                                sessionID: activeBrowserSession?.id,
                                workspaceID: AgentWorkspaceManager.downloadedSiteWorkspaceID,
                                relativePath: nil
                            )
            ...
            if let session = activeBrowserSession {
                contextRow(title: "Page", value: "\(session.title) \(session.currentURL)")
                if let summary = session.pageSummary?.nonEmpty {
                    Text(summary)
                        .font(.system(size: 12))
                        .foregroundStyle(.secondary)
                        .lineLimit(5)
                }
                ManagedBrowserWebView(sessionID: session.id)
                    .frame(height: 320)
    ...
    private var logsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack {
                Button("Copy") {
                    UIPasteboard.general.string = agentLogs.exportText()
                }
                Button("Clear") {
                    agentLogs.clear()
                    displayedLogs = []
...
@MainActor
private struct ManagedBrowserWebView: UIViewRepresentable {
    let sessionID: UUID

    func makeUIView(context: Context) -> BrowserWebContainerView {
        BrowserWebContainerView()
    }

    func updateUIView(_ container: BrowserWebContainerView, context: Context) {
        let webView = BrowserSessionManager.shared.webView(for: sessionID)
        container.host(webView: webView)
    }
}

private final class BrowserWebContainerView: UIView {
